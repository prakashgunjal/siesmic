...
(function() {
    ...
    if (includeCSS) {
        document.write('<link rel="stylesheet" type="text/css" href="' + path + '/seismic/css/ext-all' + suffix + '-debug.css"/>');
    }
    document.write('<script type="text/javascript" src="' + path + '/seismic/extjs/ext-all' + (rtl ? '-rtl' : '') + '.js"></script>');
    ...
    if (neptune) {
        ...
        neptunePath = (repoDevMode ? path + '/..' : path) +      
            '/seismic/ext-theme-neptune/ext-theme-neptune' +
            (repoDevMode ? '-dev' : '') + '.js';
