         ...
         $(document).ready(function () {
            ...
            $("#projectmilegrid").jqxGrid(
            {
                ...
                columns: [
                  {
                      ...
                      rendered: function (element) {
                          ...
                          $(element).on('change', function (event) {
                              ...
                              var pageinfo = $("#projectmilegrid").jqxGrid('getpaginginformation');
                              var pagenum = pageinfo.pagenum;
                              var pagesize = pageinfo.pagesize;
                              if (checked == null || updatingCheckState) return;
                              $("#projectmilegrid").jqxGrid('beginupdate');

                              // select all rows when the column's checkbox is checked.
                              if (checked) {
                                  $("#projectmilegrid").jqxGrid('selectallrows');
                              }
                              // unselect all rows when the column's checkbox is checked.
                              else if (checked == false) {
                                  $("#projectmilegrid").jqxGrid('clearselection');
                              }
                              ...
                              for (var i = startrow; i < startrow + pagesize; i++) {
                                  ...
                                  var boundindex = $("#projectmilegrid").jqxGrid('getrowboundindex', i);
                                  $("#projectmilegrid").jqxGrid('setcellvalue', boundindex, 'available', event.args.checked);
                              }

                              $("#projectmilegrid").jqxGrid('endupdate');
                          });
                          return true;
                      }
                  },
               ...
               renderstatusbar: function (statusbar) {
                   ...
                   deleteButton.click(function (event) {
                       var selectedrowindex = $("#projectmilegrid").jqxGrid('getselectedrowindex');
                       var rowscount = $("#projectmilegrid").jqxGrid('getdatainformation').rowscount;
                       if (selectedrowindex >= 0 && selectedrowindex < rowscount) {
                           var id = $("#projectmilegrid").jqxGrid('getrowid', selectedrowindex);
                           $("#projectmilegrid").jqxGrid('deleterow', id);
                       }
                   });
               },
            });
            
            $("#projectmilegrid").on('columnresized', function (event) {
            ...
            var updatePageState = function (pagenum) {
                var datainfo = $("#projectmilegrid").jqxGrid('getdatainformation');
                ...
                for (var i = startrow; i < startrow + pagesize; i++) {
                    var boundindex = $("#projectmilegrid").jqxGrid('getrowboundindex', i);
                    var value = $("#projectmilegrid").jqxGrid('getcellvalue', boundindex, 'available');
                    if (value) checkedItemsCount++;
                    if (value) {
                        $("#projectmilegrid").jqxGrid('selectrow', boundindex);
                    }
                    else {
                        $("#projectmilegrid").jqxGrid('unselectrow', boundindex);
                    }
                }
            ...
            $("#projectmilegrid").on('sort', function (event) {
                updatePageState();
            });

             // update the selection after page change.
            $("#projectmilegrid").on('pagechanged', function (event) {
                 updatePageState();
             });

             // select or unselect rows when a checkbox is checked or unchecked. 
             $("#projectmilegrid").on('cellvaluechanged', function (event) {
                 if (event.args.value) {
                     $("#projectmilegrid").jqxGrid('selectrow', event.args.rowindex);
                 }
                 else {
                     $("#projectmilegrid").jqxGrid('unselectrow', event.args.rowindex);
                 }
                 ...
                 if (columnCheckBox) {
                     ...
                     var pagesize = datainfo.paginginformation.pagesize;
                     var pagenum = datainfo.paginginformation.pagenum;
                     var selectedRows = $("#projectmilegrid").jqxGrid('getselectedrowindexes');
                     var state = false;
                     var count = 0;
             ...
             $("#projectmilegrid").on('contextmenu', function () {
                 return false;
             });
             ...
             $("#Menu").on('itemclick', function (event) {
                 var args = event.args;
                 var rowindex = $("#projectmilegrid").jqxGrid('getselectedrowindex');
                 if ($.trim($(args).text()) == "Edit Selected Row") {
                     editrow = rowindex;
                     var offset = $("#projectmilegrid").offset();
                     $("#popupWindow").jqxWindow({ position: { x: parseInt(offset.left) + 60, y: parseInt(offset.top) + 60} });

                     // get the clicked row's data and initialize the input fields.
                     var dataRecord = $("#projectmilegrid").jqxGrid('getrowdata', editrow);
                     ...
                     var rowid = $("#projectmilegrid").jqxGrid('getrowid', rowindex);
                     $("#projectmilegrid").jqxGrid('deleterow', rowid);
                 }
             });
 			
 			$("#projectmilegrid").on('rowclick', function (event) {
