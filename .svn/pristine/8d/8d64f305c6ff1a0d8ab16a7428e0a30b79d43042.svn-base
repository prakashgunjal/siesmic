...
public class QuoteController {
	...
	@ResponseStatus(HttpStatus.OK)
	public @ResponseBody
	List<KeyValueMap> getAccountContact(
			@RequestParam(value = "accountId") Long accountId) {

		List<KeyValueMap> keyValueMaps = contactRepository
				.getAccountAndDepartment(accountId);
		return keyValueMaps;
	}
	...
	Page<Quote> getAllQuoteList(@RequestParam int pagesize,
		...
		list = quoteService.viewQuoteDetails(quoteNumber, quoteDate,
				contactName, estimatedValue, condition, contactByLeadSalesRep,
				pagesize, pagenum,
				request.getParameter("sortorder") == null ? "modifiedAt"
						: request.getParameter("sortorder"), request
						.getParameter("sortdatafield") == null ? "modifiedAt"
						: request.getParameter("sortdatafield"));
	...
	List<SalesOrder> salesFormListPerQuote(@PathVariable Long quoteId) {
		List<SalesOrder> saleOrderLIst = quoteService
				.getSalesFormListByQuote(quoteId);
	...
	@ResponseStatus(HttpStatus.OK)
	public @ResponseBody
	ExtJSResponse createQuoteProductDetails(@PathVariable Long quoteId,
			@RequestBody Map<String, Object> reqMap) throws IOException {
		Long productId = Long.valueOf(reqMap.get("productId").toString());
		String productCategoryId = reqMap.get("productCategoryId").toString();
		String productSubCategoryId = reqMap.get("productSubCategoryId")
				.toString();
		...
		if (editFlag == "false") {
			...
			productDetailId = Long.valueOf(reqMap.get("productDetailId")
					.toString());
			if (productDetailId > 0) {
				quoteProductDetail = quoteProductDetailService
						.findById(productDetailId);
				System.out
						.println("========updating======= quoteProductDetail="
								+ quoteProductDetail);
				System.out
						.println("========updating=========QuoteProdDetailId="
								+ quoteProductDetail.getQuoteProdDetailId());
			}
		}
		if (quoteId > 0) {
	...
	@ResponseStatus(HttpStatus.OK)
	public @ResponseBody
	Quote randomNumberGenerationforQuoteOnLoad() throws FileNotFoundException,
			IOException {

		System.out
				.println("Inside randomNumberGenerationforQuoteOnLoad ==============================================>");
	...
	@ResponseStatus(HttpStatus.OK)
	public @ResponseBody
	ExtJSResponse saveQuote(@RequestParam(required = false) Long quoteId,
			@RequestParam String quoteNumber, @RequestParam String quoteDate,
			@RequestParam Long accountId, @RequestParam Long contactId,
			@RequestParam(required = false) Long opportunityId,

			@RequestParam(required = false) Long salesOrderId,
			@RequestParam String quoteSalesRep,
			@RequestParam(required = false) Long lquoteSourceId,
			@RequestParam(required = false) Long lquoteRatingId,
			@RequestParam(required = false) Long lquotePriorityId,
			@RequestParam BigDecimal estimatedValue,
			@RequestParam String closeProbability,
			@RequestParam String estimatedCloseDate,
			@RequestParam String expectedDelDate,
			@RequestParam(required = false) Long lquoteStatusId,
		...
		quote.setContactByQuoteSalesRepId(contactRepository.findOne(Long
				.parseLong(quoteSalesRep.toString())));
		if (lquoteSourceId != null && lquoteSourceId > 0) {
			ListValues listValuesBySourceId = listValuesRepository
					.findOne(lquoteSourceId);
		...
		if (lquoteRatingId != null && lquoteRatingId > 0) {
			ListValues listValuesByQuoteRatingId = listValuesRepository
					.findOne(lquoteRatingId);
		...
		if (lquotePriorityId != null && lquotePriorityId > 0) {
			ListValues listValuesByPriorityId = listValuesRepository
					.findOne(lquotePriorityId);
		...
		if (!estimatedCloseDate.isEmpty()) {
			quote.setEstimatedCloseDate(new Date(df.parse(estimatedCloseDate)
					.getTime()));
		}
		if (!expectedDelDate.isEmpty()) {
			quote.setExpectedDelDate(new Date(df.parse(expectedDelDate)
					.getTime()));
		}
		if (lquoteStatusId != null && lquoteStatusId > 0) {
			ListValues listValuesByStatusId = listValuesRepository
					.findOne(lquoteStatusId);
		...
		if (lshippingMethodId != null && lshippingMethodId > 0) {
			ListValues listValuesByShippingMethod = listValuesRepository
					.findOne(lshippingMethodId);
		...
		if (quoteId == null) {
			message = MessagesUtil.showMessage("Quote", quoteNumber, "Created",
					"Successfully");
		} else {
			message = MessagesUtil.showMessage("Quote", quoteNumber, "Updated",
					"Successfully");
		}
	...
	@ResponseStatus(HttpStatus.OK)
	public @ResponseBody
	Quote editCommonquoteOnLoad(@RequestParam(required = false) Long accountId)
