...
public class ContactController {
	...
	Page<Contact> contactReport(@RequestParam int pagesize,
		...
		String telephone = "";
		String emailId = "";
		String listValueDescription = "";
		Sort sort;
		ListValues contactType = new ListValues();
		if (filterscount > 0) {
			for (int i = 0; i < filterscount; i++) {
				filterdatafield[i] = request
						.getParameter("filterdatafield" + i);
				if ((filterdatafield[i].toString()).equals("contactName")) {
					cName = request.getParameter("filtervalue" + i);
				} else if ((filterdatafield[i].toString())
						.equals("listValues.listValueDescription")) {
					listValueDescription = request.getParameter("filtervalue"
							+ i);
					
				} else if ((filterdatafield[i].toString())
						.equals("contactNumber")) {
					cNumber = request.getParameter("filtervalue" + i);
				} else if ((filterdatafield[i].toString()).equals("title")) {
					title = request.getParameter("filtervalue" + i);
				} else if ((filterdatafield[i].toString()).equals("telephone")) {
					telephone = request.getParameter("filtervalue" + i);
				} else if ((filterdatafield[i].toString()).equals("emailId")) {
					emailId = request.getParameter("filtervalue" + i);
				}

			}
		}
		Page<Contact> list = null;

			/*list = contactRepository
					.findByContactNameContainingAndContactNumberContainingAndTitleContainingAndTelephoneContainingAndEmailIdContainingAndListValues(
							cName, cNumber, title, telephone, emailId,
							contactType, pageable);*/

			list = contactService.viewContactReport(cName, cNumber, title, telephone, emailId,
					listValueDescription,pagesize,pagenum,request
