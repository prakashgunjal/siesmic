    ...
    $.extend($.jqx._jqxGrid.prototype, {
        ...
        createInstance: function (args) {
            ...
            if (this.autorowheight && !this.autoheight) {
                throw new Error('jqxGrid: "autorowheight" requires setting the "autoheight" property to true!');
                this.host.remove();
                return;
            }
        ...
        _rendercolumnheaders: function () {
            ...
            $.each(this.columns.records, function (i, value) {
                ...
                if (columnitem.rendered) {
                    var result = columnitem.rendered($(columncontentcontainer[0].firstChild));
        ...
        _rendercolumnheader: function (text, align) {
            ...
            if (this.enableellipsis) {
                return '<div style="overflow: hidden; text-overflow: ellipsis; text-align: ' + align + '; margin-left: 4px; margin-right: 2px; margin-bottom: ' + margin + '; margin-top: ' + margin + ';">' + '<span style="text-overflow: ellipsis; cursor: default;">' + text + '</span>' + '</div>';
            }
        ...
        _renderrows: function (virtualsizeinfo, forceVirtualRefresh) {
            var self = this;
        ...
        _rendervisualcell: function (rendercellfunc, cellclass, selected, hasdetails, showdetails, hasgroups, groupslength, tablerow, renderrow, rendercolumn, renderedrows, scrolling) {
            ...
            if (column.cellclassname != '' && column.cellclassname) {
                if (typeof column.cellclassname == "string") {
                    ...
                    var customclassname = column.cellclassname(renderrow.boundindex, column.datafield, cellvalue, renderrow.bounddata);
                    if (customclassname) {
        ...
        _rendercell: function (me, column, row, value, tablecell, scrolling) {
            ...
            if (cachedcell) {
                if (column.columntype == "inline") {
                    ...
                    if (column.cellsrenderer != null) {
                        var newvalue = column.cellsrenderer(row.boundindex, column.datafield, value, defaultcellsrenderer, column.getcolumnproperties(), row.bounddata);
                    ...
                    if (me.host.jqxButton) {
                        if (value == "") value = false;
                        if (column.cellsrenderer != null) {
                            value = column.cellsrenderer(row.boundindex, column.datafield, value, defaultcellsrenderer, column.getcolumnproperties(), row.bounddata);
                        }
            ...
            if (column.cellsrenderer != null) {
                cellelement = column.cellsrenderer(row.boundindex, column.datafield, value, defaultcellsrenderer, column.getcolumnproperties(), row.bounddata);
            }
            else {
        ...
        _rendercheckboxcell: function (me, tablecell, column, row, value) {
            if (me.host.jqxCheckBox) {
                ...
                if ($tablecell.find('.jqx-checkbox').length == 0) {
                    tablecell.innerHTML = '<div tabIndex=0 style="position: absolute; top: 50%; left: 50%; margin-top: -7px; margin-left: -10px;"></div>';
        ...
        _renderrowdetails: function (cellclass, tablerow, renderrow, columnstart, columnend, renderedrows) {
            ...
            var detailselement = '<div role="rowgroup" style="overflow: hidden; width: 100%; height: 100%; margin-left: ' + indent + 'px;">' + renderrow.rowdetails + '</div>';
            if (this.rtl) {
                var detailselement = '<div role="rowgroup" style="overflow: hidden; width: 100%; height: 100%; margin-left: ' + 0 + 'px; margin-right: ' + indent + 'px;">' + renderrow.rowdetails + '</div>';
            }
        ...
        setcellvalue: function (row, datafield, value, refresh, sync) {
            ...
            if (datarow != null && datarow[datafield] !== value) {
                ...
                if (value == null || value.label == null) {
                    if (!column.nullable || (value != null && column.nullable)) {
    ...
    function jqxGridColumn(owner, data) {
        ...
        this._initfields = function (data) {
            if (data != null) {
                ...
                if (data.enabletooltips != undefined) {
                    this.enabletooltips = data.enabletooltips;
                }
                ...
                if (data.destroyeditor != undefined) {
                    this.destroyeditor = data.destroyeditor;
                }
                if (data.geteditorvalue != undefined) {
            ...
            var updatefromadapter = function (me) {
                ...
                if (!me.virtualmode) {
                    ...
                    if (me.grid.rendergridrows && me.totalrecords > 0) {
                        ...
                        if (recordscount > 0 && recordscount < rendergridrowsobj.endindex - rendergridrowsobj.startindex) {
