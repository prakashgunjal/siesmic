                ...
                columns: [
{
    ...
    rendered: function (element) {
        ...
        return true;
                  ...
                  { text: 'Person Called', editable: false, datafield: 'activityname', width: 200, rendered: tooltiprenderer },
                  { text: 'Notes', editable: false, datafield: 'description',  rendered: tooltiprenderer },
                  { text: 'Actions', editable: false, datafield: 'priority',width: 200, rendered: tooltiprenderer}
               ...
               renderstatusbar: function (statusbar) {
                   ...
                   deleteButton.click(function (event) {
                       var selectedrowindex = $("#commoncallgrid").jqxGrid('getselectedrowindex');
                       var rowscount = $("#commoncallgrid").jqxGrid('getdatainformation').rowscount;
                       if (selectedrowindex >= 0 && selectedrowindex < rowscount) {
                           var id = $("#commoncallgrid").jqxGrid('getrowid', selectedrowindex);
                           $("#commoncallgrid").jqxGrid('deleterow', id);
                       }
                   });
               },
            });
            
            $("#commoncallgrid").on('columnresized', function (event) {
            ...
            var updatePageState = function (pagenum) {
                var datainfo = $("#commoncallgrid").jqxGrid('getdatainformation');
                var pagenum = datainfo.paginginformation.pagenum;
                var pagesize = datainfo.paginginformation.pagesize;
                var startrow = pagenum * pagesize;
                // select the rows on the page.
                $("#commoncallgrid").jqxGrid('beginupdate');
                var checkedItemsCount = 0;
                for (var i = startrow; i < startrow + pagesize; i++) {
                    var boundindex = $("#commoncallgrid").jqxGrid('getrowboundindex', i);
                    var value = $("#commoncallgrid").jqxGrid('getcellvalue', boundindex, 'available');
                    if (value) checkedItemsCount++;
                    if (value) {
                        $("#commoncallgrid").jqxGrid('selectrow', boundindex);
                    }
                    else {
                        $("#commoncallgrid").jqxGrid('unselectrow', boundindex);
                    }
                }

                $("#commoncallgrid").jqxGrid('endupdate');
            ...
            $("#commoncallgrid").on('sort', function (event) {
                updatePageState();
            });

             // update the selection after page change.
            $("#commoncallgrid").on('pagechanged', function (event) {
                 updatePageState();
             });

             // select or unselect rows when a checkbox is checked or unchecked. 
             $("#commoncallgrid").on('cellvaluechanged', function (event) {
                 if (event.args.value) {
                     $("#commoncallgrid").jqxGrid('selectrow', event.args.rowindex);
                 }
                 else {
                     $("#commoncallgrid").jqxGrid('unselectrow', event.args.rowindex);
                 }
                 ...
                 if (columnCheckBox) {
                     ...
                     var pagesize = datainfo.paginginformation.pagesize;
                     var pagenum = datainfo.paginginformation.pagenum;
                     var selectedRows = $("#commoncallgrid").jqxGrid('getselectedrowindexes');
                     var state = false;
                     var count = 0;
