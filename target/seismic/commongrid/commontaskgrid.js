                ...
                columns: [
{
    ...
    rendered: function (element) {
        ...
        return true;
                  ...
                  { text: 'Task Date', editable: false, columntype: 'datetimeinput',cellsformat: 'd',datafield: 'activityid', width: 200, rendered: tooltiprenderer },
                  { text: 'Description', editable: false, datafield: 'activityname',  rendered: tooltiprenderer },
                  { text: 'Task Owner', editable: false, datafield: 'description',  rendered: tooltiprenderer },
                  { text: 'Status', editable: false, datafield: 'priority',width: 200, rendered: tooltiprenderer}
               ...
               renderstatusbar: function (statusbar) {
                   ...
                   deleteButton.click(function (event) {
                       var selectedrowindex = $("#commontaskgrid").jqxGrid('getselectedrowindex');
                       var rowscount = $("#commontaskgrid").jqxGrid('getdatainformation').rowscount;
                       if (selectedrowindex >= 0 && selectedrowindex < rowscount) {
                           var id = $("#commontaskgrid").jqxGrid('getrowid', selectedrowindex);
                           $("#commontaskgrid").jqxGrid('deleterow', id);
                       }
                   });
               },
            });
            
            $("#commontaskgrid").on('columnresized', function (event) {
            ...
            var updatePageState = function (pagenum) {
                var datainfo = $("#commontaskgrid").jqxGrid('getdatainformation');
                var pagenum = datainfo.paginginformation.pagenum;
                var pagesize = datainfo.paginginformation.pagesize;
                var startrow = pagenum * pagesize;
                // select the rows on the page.
                $("#commontaskgrid").jqxGrid('beginupdate');
                var checkedItemsCount = 0;
                for (var i = startrow; i < startrow + pagesize; i++) {
                    var boundindex = $("#commontaskgrid").jqxGrid('getrowboundindex', i);
                    var value = $("#commontaskgrid").jqxGrid('getcellvalue', boundindex, 'available');
                    if (value) checkedItemsCount++;
                    if (value) {
                        $("#commontaskgrid").jqxGrid('selectrow', boundindex);
                    }
                    else {
                        $("#commontaskgrid").jqxGrid('unselectrow', boundindex);
                    }
                }

                $("#commontaskgrid").jqxGrid('endupdate');
            ...
            $("#commontaskgrid").on('sort', function (event) {
                updatePageState();
            });

             // update the selection after page change.
            $("#commontaskgrid").on('pagechanged', function (event) {
                 updatePageState();
             });

             // select or unselect rows when a checkbox is checked or unchecked. 
             $("#commontaskgrid").on('cellvaluechanged', function (event) {
                 if (event.args.value) {
                     $("#commontaskgrid").jqxGrid('selectrow', event.args.rowindex);
                 }
                 else {
                     $("#commontaskgrid").jqxGrid('unselectrow', event.args.rowindex);
                 }
                 ...
                 if (columnCheckBox) {
                     ...
                     var pagesize = datainfo.paginginformation.pagesize;
                     var pagenum = datainfo.paginginformation.pagenum;
                     var selectedRows = $("#commontaskgrid").jqxGrid('getselectedrowindexes');
                     var state = false;
                     var count = 0;
