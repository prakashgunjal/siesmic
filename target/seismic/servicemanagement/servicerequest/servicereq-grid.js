         ...
         $(document).ready(function () {
            ...
            $("#servicereqgrid").jqxGrid(
            {
                ...
                columns: [
                  {
                      ...
                      rendered: function (element) {
                          ...
                          $(element).on('change', function (event) {
                              ...
                              var pageinfo = $("#servicereqgrid").jqxGrid('getpaginginformation');
                              ...
                              if (checked) {
                                  $("#servicereqgrid").jqxGrid('selectallrows');
                              }
                              // unselect all rows when the column's checkbox is checked.
                              else if (checked == false) {
                                  $("#servicereqgrid").jqxGrid('clearselection');
                              }
                              ...
                              for (var i = startrow; i < startrow + pagesize; i++) {
                                  ...
                                  $("#servicereqgrid").jqxGrid('setcellvalue', boundindex, 'available', event.args.checked);
                              }
                  ...
                  { text: 'Service Account Name', editable: false, datafield: 'serviceaccname', width: 150, rendered: tooltiprenderer },
                  { text: 'Service Product Name', editable: false, datafield: 'serviceprodname', width: 200, rendered: tooltiprenderer },
                  { text: 'Service Request Date', editable: false, datafield: 'servicereqdate', width: 200, rendered: tooltiprenderer},
				  { text: 'Priority', editable: false, datafield: 'priority',rendered: tooltiprenderer },
               ...
               renderstatusbar: function (statusbar) {
                   ...
                   deleteButton.click(function (event) {
                       var selectedrowindex = $("#servicereqgrid").jqxGrid('getselectedrowindex');
                       var rowscount = $("#servicereqgrid").jqxGrid('getdatainformation').rowscount;
                       if (selectedrowindex >= 0 && selectedrowindex < rowscount) {
                           var id = $("#servicereqgrid").jqxGrid('getrowid', selectedrowindex);
                           $("#servicereqgrid").jqxGrid('deleterow', id);
                       }
                   });
               },
            });
            ...
            $("#servicereqgrid").on('pagechanged', function (event) {
             ...
             $("#Menu").on('itemclick', function (event) {
                 var args = event.args;
                 var rowindex = $("#servicereqgrid").jqxGrid('getselectedrowindex');
                 if ($.trim($(args).text()) == "Edit Selected Row") {
                     editrow = rowindex;
                     var offset = $("#servicereqgrid").offset();
                     $("#popupWindow").jqxWindow({ position: { x: parseInt(offset.left) + 60, y: parseInt(offset.top) + 60} });

                     // get the clicked row's data and initialize the input fields.
                     var dataRecord = $("#servicereqgrid").jqxGrid('getrowdata', editrow);
                     ...
                     var rowid = $("#servicereqgrid").jqxGrid('getrowid', rowindex);
                     $("#servicereqgrid").jqxGrid('deleterow', rowid);
                 }
             });
