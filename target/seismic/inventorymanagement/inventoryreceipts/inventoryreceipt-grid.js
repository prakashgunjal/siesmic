         ...
         $(document).ready(function () {
             ...
             var source =
            {
                ...
                [
                    ...
                    { name: 'receiptqty', type: 'string' },
                    { name: 'receiptlocation', type: 'string' },
                  ...
                  /*{
                      ...
                      rendered: function (element) {
                          ...
                          $(element).on('change', function (event) {
                              ...
                              var pageinfo = $("#inventreciepgrid").jqxGrid('getpaginginformation');
                              var pagenum = pageinfo.pagenum;
                              var pagesize = pageinfo.pagesize;
                              if (checked == null || updatingCheckState) return;
                              $("#inventreciepgrid").jqxGrid('beginupdate');

                              // select all rows when the column's checkbox is checked.
                              if (checked) {
                                  $("#inventreciepgrid").jqxGrid('selectallrows');
                              }
                              // unselect all rows when the column's checkbox is checked.
                              else if (checked == false) {
                                  $("#inventreciepgrid").jqxGrid('clearselection');
                              }
                              ...
                              for (var i = startrow; i < startrow + pagesize; i++) {
                                  ...
                                  var boundindex = $("#inventreciepgrid").jqxGrid('getrowboundindex', i);
                                  $("#inventreciepgrid").jqxGrid('setcellvalue', boundindex, 'available', event.args.checked);
                              }

                              $("#inventreciepgrid").jqxGrid('endupdate');
                          });
                          return true;
                      }
                  },*/
                  { text: 'Receipt Name', editable: false, datafield: 'receiptname', width: 240, rendered: tooltiprenderer },
                  { text: 'Receipt date', editable: false, datafield: 'receiptdate', width: 200, rendered: tooltiprenderer },
                  { text: 'Receipt Type', editable: false, datafield: 'receipttype', width: 200, rendered: tooltiprenderer },
                  { text: 'Receipt Quantity', editable: false, datafield: 'receiptqty', width: 200, rendered: tooltiprenderer },
                  { text: 'Location', editable: false, datafield: 'receiptlocation',width: 200, rendered: tooltiprenderer}
            ...
            $("#inventreciepgrid").on('columnresized', function (event) {
            ...
            var updatePageState = function (pagenum) {
                var datainfo = $("#inventreciepgrid").jqxGrid('getdatainformation');
                ...
                for (var i = startrow; i < startrow + pagesize; i++) {
                    var boundindex = $("#inventreciepgrid").jqxGrid('getrowboundindex', i);
                    var value = $("#inventreciepgrid").jqxGrid('getcellvalue', boundindex, 'available');
                    if (value) checkedItemsCount++;
                    if (value) {
                        $("#inventreciepgrid").jqxGrid('selectrow', boundindex);
                    }
                    else {
                        $("#inventreciepgrid").jqxGrid('unselectrow', boundindex);
                    }
                }
            ...
            $("#inventreciepgrid").on('sort', function (event) {
                updatePageState();
            });

             // update the selection after page change.
            $("#inventreciepgrid").on('pagechanged', function (event) {
                 updatePageState();
             });

             // select or unselect rows when a checkbox is checked or unchecked. 
             $("#inventreciepgrid").on('cellvaluechanged', function (event) {
                 if (event.args.value) {
                     $("#inventreciepgrid").jqxGrid('selectrow', event.args.rowindex);
                 }
                 else {
                     $("#inventreciepgrid").jqxGrid('unselectrow', event.args.rowindex);
                 }
                 ...
                 if (columnCheckBox) {
                     ...
                     var pagesize = datainfo.paginginformation.pagesize;
                     var pagenum = datainfo.paginginformation.pagenum;
                     var selectedRows = $("#inventreciepgrid").jqxGrid('getselectedrowindexes');
                     var state = false;
                     var count = 0;
             ...
             $("#inventreciepgrid").on('contextmenu', function () {
                 return false;
             });
             ...
             $("#Menu").on('itemclick', function (event) {
                 var args = event.args;
                 var rowindex = $("#inventreciepgrid").jqxGrid('getselectedrowindex');
                 if ($.trim($(args).text()) == "Edit Selected Row") {
                     editrow = rowindex;
                     var offset = $("#inventreciepgrid").offset();
                     $("#popupWindow").jqxWindow({ position: { x: parseInt(offset.left) + 60, y: parseInt(offset.top) + 60} });

                     // get the clicked row's data and initialize the input fields.
                     var dataRecord = $("#inventreciepgrid").jqxGrid('getrowdata', editrow);
                     ...
                     var rowid = $("#inventreciepgrid").jqxGrid('getrowid', rowindex);
                     $("#inventreciepgrid").jqxGrid('deleterow', rowid);
                 }
             });
 			
 			$("#inventreciepgrid").on('rowclick', function (event) {
