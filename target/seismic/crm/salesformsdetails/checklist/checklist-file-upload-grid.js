         ...
         $(document).ready(function () {
            ...
            $("#docuploadgrid").jqxGrid(
            {
                ...
                columns: [
                  ...
                  { text: 'Document Title', editable: false, datafield: 'activityname', width: 200, rendered: tooltiprenderer },
                  { text: 'Attachments', editable: false, datafield: 'priority', rendered: tooltiprenderer}
               ...
               /*renderstatusbar: function (statusbar) {
                   ...
                   addButton.click(function (event) {
                	   var newwindow;
                	   newwindow=window.open('../../GenericJspPages/document/document.jsp','_parent','height=400,width=1000');   // it takes lotsof more arguments you can use as per your needs  
            ...
            $("#docuploadgrid").on('columnresized', function (event) {
            ...
            var updatePageState = function (pagenum) {
                var datainfo = $("#docuploadgrid").jqxGrid('getdatainformation');
                var pagenum = datainfo.paginginformation.pagenum;
                var pagesize = datainfo.paginginformation.pagesize;
                var startrow = pagenum * pagesize;
                // select the rows on the page.
                $("#docuploadgrid").jqxGrid('beginupdate');
                var checkedItemsCount = 0;
                for (var i = startrow; i < startrow + pagesize; i++) {
                    var boundindex = $("#docuploadgrid").jqxGrid('getrowboundindex', i);
                    var value = $("#docuploadgrid").jqxGrid('getcellvalue', boundindex, 'available');
                    if (value) checkedItemsCount++;
                    if (value) {
                        $("#docuploadgrid").jqxGrid('selectrow', boundindex);
                    }
                    else {
                        $("#docuploadgrid").jqxGrid('unselectrow', boundindex);
                    }
                }

                $("#docuploadgrid").jqxGrid('endupdate');
            ...
            $("#docuploadgrid").on('sort', function (event) {
                updatePageState();
            });

             // update the selection after page change.
            $("#docuploadgrid").on('pagechanged', function (event) {
                 updatePageState();
             });

             // select or unselect rows when a checkbox is checked or unchecked. 
             $("#docuploadgrid").on('cellvaluechanged', function (event) {
                 if (event.args.value) {
                     $("#docuploadgrid").jqxGrid('selectrow', event.args.rowindex);
                 }
                 else {
                     $("#docuploadgrid").jqxGrid('unselectrow', event.args.rowindex);
                 }
                 ...
                 if (columnCheckBox) {
                     ...
                     var pagesize = datainfo.paginginformation.pagesize;
                     var pagenum = datainfo.paginginformation.pagenum;
                     var selectedRows = $("#docuploadgrid").jqxGrid('getselectedrowindexes');
                     var state = false;
                     var count = 0;
