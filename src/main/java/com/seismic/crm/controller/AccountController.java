...
public class AccountController {
	...
	public @ResponseBody
	Page<Account> getAllAccountData(
			@RequestParam int pagesize,@RequestParam int pagenum, @RequestParam int filterscount,
		...
		if (filterscount > 0) {
			for (int i = 0; i < filterscount; i++) {
				...
				if ((filterdatafield[i].toString()).equals("accountName")) {
					accountName = request.getParameter("filtervalue" + i);
				} else if ((filterdatafield[i].toString())
						.equals("accountNumber")) {
					accountNumber = request.getParameter("filtervalue"
							+ i);

				} else if ((filterdatafield[i].toString())
						.equals("contactByPrimaryContactName.contactName")) {
					primaryContactName = request.getParameter("filtervalue" + i);
				} else if ((filterdatafield[i].toString())
						.equals("primaryTelephoneNumber")) {
					primaryTelephoneNumber = request.getParameter("filtervalue" + i);
				} else if ((filterdatafield[i].toString())
						.equals("contact.contactName")) {
					salesrep = request.getParameter("filtervalue"
							+ i);
				}
		...
		list = accountService.viewAccountDetails(accountName,accountNumber,primaryContactName,primaryTelephoneNumber,
				salesrep,pagesize,pagenum,request
		...
		if(list.hasContent()){
		  ...
		  for (Account account:list){
			   if(account.getContactByPrimaryContactName()!=null){
