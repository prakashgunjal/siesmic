    ...
    $.extend($.jqx._jqxSplitter.prototype, {
        ...
        _addHandlers: function () {
            ...
            $.jqx.utilities.resize(this.host, function () {
                if ((that.width && that.width.toString().indexOf('%') != -1) || (that.height && that.height.toString().indexOf('%') != -1)) {
                    that._layoutPanels();
                }
            ...
            if (!this._displayTimer) {
                if (this._hiddenParent()) {
                    this._displayTimer = setInterval(function () {
                        try
                        {
                            if (!that._hiddenParent()) {
                                clearInterval(that._displayTimer);
                                that._displayTimer = 0;
                                that.refresh();
                            }
                        }
                        catch (error) {
